...
- **D11 — Context-scoped chip registry (new, Artur):** advisor quick-chips are **scoped to where you are**, never mixed. One registry `CHIPS = {wall, piece, pick}`, each chip `{label, run, when?}` with an **explicit action** (no text-parse round-trip); `refreshQuick()` renders the current scope (from `wall.mode`+`fp()`) and is called after every action, so the chat stays continuous but the chips always match context. **Root bug fixed:** `respond()` used to reset chips to the wall set after every message, flipping piece-context chips to wall ones. **Scopes:** WALL = compose (Surprise-if-empty · Add a piece · Another layout* · Remove a piece*); PIECE = refine this artwork (Swap artwork · Warmer · Darker · Add a frame — **no "Back to wall" chip**, the ✕ does that); PICK = choose (Surprise · Match the rest · Bring-back*). `*` = conditional (`when`). **Corrections applied:** Warmer/Darker are **piece-scope only** (they act on the focused piece, not the wall — and today they're a swap gesture; real palette shift = MVP/D7). Extensible: adding a chip = one registry entry (more chips are coming). Built on the mockup — flow/logic, not a mechanism.
- **D10 — Unified slot flow (new, Artur):** clicking **any** slot opens the **same overlay** — filled → *detail* (swap/frame/warmer); empty → *pick* mode (no hero artwork; a "pick a piece" placeholder + the **global Recently-viewed history** + the catalog, both orientation-filtered to what fits this slot). Picking a tile fills the slot and transitions **in place** to detail (one flow, no jump back to the wall). Mental model: **wall = compose** (add/remove/layout); **a piece = change what's on it**. Fixes a real gap — empty slots never opened the picker, and the "Browse the catalog" chip was a fake auto-fill. *(Built on the mockup — it's flow/design, not a mechanism.)*
