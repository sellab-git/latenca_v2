...
### D-051 refinement (2026-07-22) — no banners; every slot is one of our shapes
Artur spotted that some layouts contained a **wide-thin "banner" slot** (Mixtiles' one elongated title graphic, real aspect ~3.0) — a shape we don't sell (we only offer P≈0.71 / S≈1.0 / L≈1.4). Rule: **every slot must be one of our three shapes.** The generator (`shared/gen-wall-layouts.py`) now runs a `debanner` pass: any slot with real aspect (=(w/h)×layoutAspect) outside `[0.6, 1.55]` is **reworked into a centred landscape (1.4)** — or portrait (0.714) if too tall — occupying the same band, so the layout survives with the same piece count (Artur preferred rework over deletion; delete only if it can't be done logically — here all 6 reworked cleanly). Affected: 5-piece L3 (Embraced by Warmth), 9-L1 (Framed with Joy), 10-L2 (Timeless Moments), 10-L4 (Framed Connections), 11-L1 (Loving Life), 13-L1 (Frame by Frame). Also fixed the invented 1-/2-piece fallbacks (the single tile was rendering as a distorted tall bar; now a clean 50×70 portrait). Frozen `05-wall` v9. The full 45-wall matrix (`mixtiles-positions.json`) stays intact for designing new layouts later.

### D-051 curation (2026-07-22) — Artur layout review; end at 12 pieces
A curation layer in `shared/gen-wall-layouts.py` (DROP set + MODIFY dict, by layout name):
- **Collapse look-alikes:** the 6-piece list had three identical 3×2 square grids (Dancing Lights, Heartfelt Reflections, 6 Enduring Pictures) — same layout, only tiny proportion differences; keep one (6 Enduring). Our exact-position dedup missed these; they're the "same layout, different size" case. 6-piece is now 4 real variants: 3×2 squares · 3×2 portraits · SPSPSP alternating · Story-Wall cluster.
- **Banner→square where the layout is a square grid** (Artur preferred a matching square over the generic landscape rework): 10-piece *Timeless Moments* bottom element → grid-size square; 10-piece *Framed Connections* top element → square (necessarily **small** — the 3×3 grid fills the height, only a thin top band remains; flagged as keep-small-or-drop).
- **11-piece *Loving Life*:** the middle-row side squares stuck out past the grid → aligned to columns 1 & 4, so the centre landscape is framed cleanly.
